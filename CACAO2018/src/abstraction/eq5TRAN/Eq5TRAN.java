...
public class Eq5TRAN implements Acteur, IAcheteurPoudre, IVendeurPoudre, IvendeurOccasionnelChocoTer, IAcheteurFeveV4, IVendeurChocoBis {
    ...
    public List<ContratFeveV3> getResultVentesV3() {
        ...
        for (ContratFeveV3 c : listeContrat) {
            if ((c.getProposition_Prix() <= c.getDemande_Prix()) && c.getProposition_Quantite() <= c.getDemande_Quantite() && c.getProposition_Quantite()!=0) {
                c.setReponse(true); 
                journal.ajouter("L'équipe 5 a conclu une affaire avec "+((Acteur)c.getProducteur()).getNom()+" Quantité échangée :" + c.getProposition_Quantite()+ "T de " + c.getQualite()+" au prix de : "+(int)c.getProposition_Prix()+ "euros par T ");
    ...
    public double getReponseTer(DemandeAO d) {
        switch (d.getQualite()) {
            ...
            case 4:
                if (d.getQuantite() < 0.2 * stocks[TABLETTES_BQ].getValeur()*(1_000_000 / 0.2)) {
            ...
            case 5:
                if (d.getQuantite() < 0.2 * stocks[TABLETTES_MQ].getValeur()*(1_000_000 / 0.2)) {
            ...
            case 6:
                if (d.getQuantite() < 0.2 * stocks[TABLETTES_HQ].getValeur()*(1_000_000 / 0.2)) {
    ...
    public void envoyerReponseTer(Acteur acteur, int quantite, int qualite, double prix) {
        this.depenser(-prix);
        this.stocks[qualite].setValeur(this, this.stocks[qualite].getValeur() - quantite /(1_000_000 / 0.2));
